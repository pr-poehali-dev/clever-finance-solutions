...
def handler(event: dict, context) -> dict:
    """Управление каналами: создание, вступление, список, сообщения"""
    ...
    if path.endswith("/create") and method == "POST":
        ...
        cur.execute(
            "INSERT INTO channels (name, description, owner_id, invite_code) VALUES (%s, %s, %s, %s) RETURNING id, invite_code",
    ...
    if path.endswith("/list") and method == "GET":
        ...
        return {"statusCode": 200, "headers": headers, "body": json.dumps([
            {"id": r[0], "name": r[1], "description": r[2], "invite_code": r[3], "owner_id": r[4], "member_count": r[5]}
            for r in rows
        ])}
    ...
    if path.endswith("/members") and method == "GET":
        ...
        conn.close()
        return {"statusCode": 200, "headers": headers, "body": json.dumps([
            {"id": r[0], "username": r[1], "display_name": r[2]} for r in rows
        ])}
    ...
    if path.endswith("/send") and method == "POST":
        ...
        cur.execute(
            "INSERT INTO channel_messages (channel_id, sender_id, content) VALUES (%s, %s, %s) RETURNING id, created_at",
